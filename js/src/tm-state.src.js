...
function renderizarMasVendidos() {
    ...
    productosAMostrar.forEach(producto => {
        const card = document.createElement('div');
        card.className = 'producto-card tm-card-premium tm-anim-card';
        ...
        if (_esAgotado) {
            ...
            if (producto.masVendido) _badgesHtml += '<span class="tm-card-badge tm-badge-top">🔥 Top</span>';
            if (_stock === 1) _badgesHtml += '<span class="tm-card-badge tm-badge-ultima tm-pulse">⏰ Última</span>';
        ...
        if (_specs.length && !_esAgotado) {
            ...
            _specs.forEach(function(s) { _specsHtml += '<span class="tm-card-spec">' + escapeHtml(String(s)) + '</span>'; });
            _specsHtml += '</div>';
        }
        ...
        if (!_esAgotado) {
            ...
            _stockBarHtml = '<div class="tm-card-stockbar"><div class="tm-card-stockbar-label"><span>Stock</span><span>' + _stock + ' uds</span></div><div class="tm-card-stockbar-track"><div class="tm-card-stockbar-fill" style="width:' + _stockPct + '%;background:' + _stockColor + '"></div></div></div>';
        }
        ...
        let _trustHtml = '<div class="tm-card-trust"><span class="tm-trust-icon" title="Pago contra entrega">🔒</span>';
        ...
        card.innerHTML =
            ...
            '<div class="tm-card-body">' +
                ...
                '<div class="tm-card-precio">' +
                    ...
                    '<div class="tm-card-precio-usd"><span class="precio-actual" data-usd="' + safeNum(producto.precioActual) + '">$' + Number(producto.precioActual).toFixed(2) + ' USD</span>' + (_hasDescuento ? ' <span class="tm-card-precio-orig">$' + _precioOrigUsd.toFixed(0) + '</span>' : '') + '</div>' +
                '</div>' +
                (_esAgotado
